...
export default function Login() {
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-16" data-testid="login-page">
      <h1 className="font-display text-4xl font-semibold text-[#2A1F26]">Welcome Back</h1>
      <p className="text-[#5A4A52] mt-2">Login to track your orders.</p>
      <form onSubmit={onSubmit} className="brut-card p-6 mt-6 space-y-4">
        ...
        <div className="text-sm text-center text-[#5A4A52]">No account? <Link to="/signup" className="font-medium text-[#C68F8F] underline">Sign up</Link></div>
      </form>
      <div className="text-center mt-4 text-sm text-[#8B7B81]">Or <Link to="/products" className="underline font-medium text-[#C68F8F]">continue as guest</Link></div>
    </div>
  );
}
